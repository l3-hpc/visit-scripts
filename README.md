...
VisIt can usually generate a nice pseduocolor plot with just about any dataset.  If you open the dataset and see a bunch of weird and crazy garbage, check the [VisIt for SCHISM](https://youtu.be/mvCu8vCbTLs) video, which uses a threshold operator to remove garbage.  (Usually 'garbage' is not really garbage, just missing or NAN cells due to land masks or wetting and drying.)
