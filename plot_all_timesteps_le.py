...
#So it can find l3v, leem_vars, and setpaths 
##--Set path to scripts----
...
FILE_NAME = os.path.join(OUTPUTS,"Baseline_2013_0001.nc")
...
for var in leem_vars:
    #Create a directory for images if one doesn't exist.
    #Note, existing files will be overwritten
    IMGS_DIR = os.path.join(IMAGES,var['name']) 
...
#Just for reference, definition of TP based on LEEM vars
#DefineScalarExpression("TP_tot", "RPOP + LPOP + RDOP + LDOP + PO4T + LPIP + RPIP + (ZOO1 + ZOO2 + ZOO3)/50.0")

## Define base attributes for saving images 
...
for plot in which_plots:
    for var in leem_vars:
        ...
        for state in range(totalstates):
            ...
            SaveWindowAtts.outputDirectory = os.path.join(IMAGES,setvars['var'])
            SaveWindowAtts.fileName = setvars["name"] + "_" + plot  + FILE_TS
            if(state==0):
	        print("outputDirectory")
	        print(SaveWindowAtts.outputDirectory)
	        print("fileName")
